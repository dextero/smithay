...
impl VulkanImport {
    ...
    pub unsafe fn import_dmabuf(
        ...
        let (vk_format, wgpu_format) = match format.code {
            ...
            smithay::backend::allocator::Fourcc::Argb8888 => (vk::Format::B8G8R8A8_UNORM, wgpu::TextureFormat::Bgra8Unorm),

            smithay::backend::allocator::Fourcc::Xrgb8888 => (vk::Format::B8G8R8A8_UNORM, wgpu::TextureFormat::Bgra8Unorm),

            smithay::backend::allocator::Fourcc::Abgr8888 => (vk::Format::R8G8B8A8_UNORM, wgpu::TextureFormat::Rgba8Unorm),

            smithay::backend::allocator::Fourcc::Xbgr8888 => (vk::Format::R8G8B8A8_UNORM, wgpu::TextureFormat::Rgba8Unorm),

            _ => (vk::Format::R8G8B8A8_UNORM, wgpu::TextureFormat::Rgba8Unorm),
        ...
        let planes = dmabuf.offsets().zip(dmabuf.strides()).enumerate().map(|(_idx, (offset, stride))| {
        ...
        let memory_type_index = self.find_memory_type(memory_requirements.memory_type_bits, vk::MemoryPropertyFlags::DEVICE_LOCAL)

            .unwrap_or_else(|| self.find_memory_type(memory_requirements.memory_type_bits, vk::MemoryPropertyFlags::empty()).unwrap());
        ...
        let hal_device = wgpu_device.as_hal::<hal::api::Vulkan>().expect("Not using Vulkan");
